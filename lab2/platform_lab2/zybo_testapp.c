...
void filter_or_bypass_input(void)
{
	...
	while (1)
	{
		...
		if(sw_check & 01) // SW0=1 then pass the sample through the filter
		{
			...
			XFir_Set_data_in(&HlsFir_left, u32DataL >> 8);
			XFir_Set_data_in(&HlsFir_right, u32DataR >> 8);
			// Clear done flags
		    ...
		    u32DataL = XFir_Get_data_out(&HlsFir_left);
		    while(!ResultAvailHlsFir_right);
		    u32DataR = XFir_Get_data_out(&HlsFir_right);
